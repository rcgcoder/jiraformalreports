				...
				self.addStep("Retrieving Sprints of projects",function(){
					...
					self.addStep("Creating Boards",function(arrBoards){
						arrBoards.forEach(function(srcBoard){
							...
							var prjId="";
							if (isDefined(lct)&&(isDefined(lct.projectKey))){
								var prjKey=lct.projectKey;
								if (self.projects.exists(prjKey)){
...
//			if (self.isReusingIssueList()||self.isReusingReport()) return;
		...
		self.addStep("Replacing all img tags to img crossorigin=Anonymous",function(sModelProcessedResult){
            debugger;
            var strTgt="<img ";
            var sReplace='<img crossorigin="use-credentials" ';
            ...
            var sReplace='<jira-attachment-thumbnail crossorigin="use-credentials" ';
            var saAux=sModelProcessedResult;
            var iPosAnt=0;
            var iPos=saAux.saFindPos(strTgt,false,0);
		    while ((iPos>=0)&&(iPos>=iPosAnt)){
